...
const callPattern =
  /\.(call|send|delegatecall|transfer)\s*(?:\{[^}]*\})?\s*\(/g;
const checkedExpression =
  /\b(?:require|assert|if)\s*\([^;]*\.(?:call|send|delegatecall|transfer)\b/;
const assignedResult =
  /(?:^|[;(])\s*(?:\([^)]*\)|(?:bool|bytes(?:\s+memory)?|uint(?:\d+)?|int(?:\d+)?|address|bytes\d+)\s+\w+)\s*=\s*[^;]*\.(?:call|send|delegatecall|transfer)\b/;
...
export class UncheckedReturnPlugin implements IRulePlugin {
      ...
      for (const match of line.matchAll(callPattern)) {
        ...
        findings.push({
          ...
          description:
            ...
            method +
            '() is ignored. A failed low-level call may leave the contract in an unsafe state.',
          ...
          codeSnippet: trimmed,
          recommendation:
            'Capture the boolean result and enforce require(success) before continuing.',
  ...
  getFixRecommendation(finding: FindingResult): string {
    return (
      'Capture the call result before continuing, then use require(success) ' +
      '(or an equivalent explicit error path) at ' +
      finding.filePath +
      ':' +
      finding.lineStart +
      '.'
